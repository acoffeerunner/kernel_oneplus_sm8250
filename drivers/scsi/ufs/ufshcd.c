			...
			struct ufshcd_cmd_log_entry *entry)
{
	...
	u8 opcode = 0;
	u32 intr, doorbell;
	struct ufshcd_lrb *lrbp = &hba->lrb[entry->tag];
	...
	if (!trace_ufshcd_command_enabled()) {
		...
		if (cmd)
			ufshcd_add_cmd_upiu_trace(hba, entry->tag, entry->str);
		return;
	}
...
int ufshcd_send_command(struct ufs_hba *hba, unsigned int task_tag)
{
	...
	ufshcd_cond_add_cmd_trace(hba, task_tag, hba->lrb[task_tag].cmd ? "scsi_send" : "dev_cmd_send");
